...
def compute_policy_loss_seq_rloo(
    ...
    """
    Compute sequence-level PPO loss for RLOO-style advantages.

    This keeps the old seq_rloo behavior: aggregate token log-ratios by summing
    over the valid response tokens, then exponentiate the sequence sum. Unlike
    GSPO, this does not normalize the sequence log-ratio by response length.
    """
    ...
    negative_approx_kl = log_prob - old_log_prob
    seq_negative_approx_kl = verl_F.masked_sum(negative_approx_kl, response_mask, axis=-1)
    seq_negative_approx_kl = torch.clamp(seq_negative_approx_kl, min=-20.0, max=20.0)
    seq_ratio = torch.exp(seq_negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    # seq_rloo expects one scalar advantage per response sequence.
    valid_cmp = torch.isclose(advantages, advantages[:, :1]).masked_fill(~valid_response_mask, True)
    ...
    pg_losses2 = -seq_advantages * torch.clamp(seq_ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    pg_clipfrac = torch.mean(torch.gt(pg_losses2, pg_losses1).float())

    pg_losses3 = -seq_advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = torch.mean(torch.gt(clip_pg_losses1, pg_losses3).float() * (seq_advantages < 0).float())
...
@register_policy_loss("fspo")
def compute_policy_loss_fspo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """
    Compute FSPO loss with sequence-level log-space clipping.

    Compared with seq_rloo, FSPO clips the sequence log-ratio first and then
    exponentiates it back to ratio space. The clipping bandwidth scales with
    sqrt(response_length), using sigma=0.03 by default.
    """
    ...
    # FSPO length-adaptive log-space clipping: b_L = z * sigma * sqrt(L).
    fspo_clip_sigma = config.policy_loss.get("fspo_clip_sigma", 0.01)
    fspo_clip_z = config.policy_loss.get("fspo_clip_z", 1.0)
    seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1).to(log_prob.dtype)
    log_clip_bound = fspo_clip_z * fspo_clip_sigma * torch.sqrt(seq_lengths)
    clipped_seq_negative_approx_kl = torch.clamp(
        seq_negative_approx_kl,
        min=-log_clip_bound,
        max=log_clip_bound,
    )
    clipped_seq_ratio = torch.exp(clipped_seq_negative_approx_kl)

    # FSPO expects one scalar advantage per response sequence.
    valid_cmp = torch.isclose(advantages, advantages[:, :1]).masked_fill(~valid_response_mask, True)
    assert valid_cmp.all(), "Each advantage should be the same within each sequence"
    seq_advantages = advantages[:, 0]

    pg_losses1 = -seq_advantages * seq_ratio
    pg_losses2 = -seq_advantages * clipped_seq_ratio
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    pg_clipfrac = torch.mean(torch.gt(pg_losses2, pg_losses1).float())
...
# @register_policy_loss("fspo_only_up")
# def compute_policy_loss_fspo_only_up(
#     old_log_prob: torch.Tensor,
#     log_prob: torch.Tensor,
#     advantages: torch.Tensor,
#     response_mask: torch.Tensor,
...
# ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
#     """
#     Compute a sequence-level FSPO variant with only the upper clip bound enabled.

#     This variant keeps the same sequence-level objective and dual-clip branch as
#     FSPO, but only caps the importance ratio at 1 + clip_ratio_high. The lower
#     side is left unclipped.
#     """
...
#     upper_clip_ratio = 1 + clip_ratio_high
#     clipped_seq_ratio = torch.clamp(seq_ratio, max=upper_clip_ratio)
...
@register_policy_loss("tic-grpo")
def compute_policy_loss_fspo_only_up_ori(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """
    Compute an FSPO variant with only the original log-space upper clip bound enabled.

    This keeps the original FSPO length-adaptive log-space clipping rule, but
    removes the lower bound and only caps the sequence log-ratio at +b_L.
    """
    ...
    # Original FSPO upper clipping in log space: S_clipped = min(S, b_L).
    ###改动
    fspo_clip_sigma = config.policy_loss.get("fspo_clip_sigma", 0.01)
    fspo_clip_z = config.policy_loss.get("fspo_clip_z", 1.0)
    seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1).to(log_prob.dtype)
    ####改动
    log_clip_bound = fspo_clip_z * fspo_clip_sigma * torch.sqrt(seq_lengths)
    clipped_seq_negative_approx_kl = torch.minimum(seq_negative_approx_kl, log_clip_bound)
    clipped_seq_ratio = torch.exp(clipped_seq_negative_approx_kl)
...
@register_policy_loss("fspo_only_up_ori_o")
def compute_policy_loss_fspo_only_up_ori(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    ...
    # Original FSPO upper clipping in log space: S_clipped = min(S, b_L).
    fspo_clip_sigma = config.policy_loss.get("fspo_clip_sigma", 0.03)
    fspo_clip_z = config.policy_loss.get("fspo_clip_z", 1.0)
    seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1).to(log_prob.dtype)
    log_clip_bound = fspo_clip_z * fspo_clip_sigma * torch.sqrt(seq_lengths)
    clipped_seq_negative_approx_kl = torch.clamp(seq_negative_approx_kl,torch.zeros_like(log_clip_bound), log_clip_bound)
    clipped_seq_ratio = torch.exp(clipped_seq_negative_approx_kl)
